...
rova_reg <- rova +  
  ...
  {if(nrow(rova_sf %>% filter(totreg_std2 == 0))>0)
...
jpeg("ROVA_grid.jpg", height = 8, width = 10.5, units = "in", res = 600)
grid.newpage()
pushViewport(map_vp)
...
print(vama_reg, viewport(layout.pos.row = 1, 
                         layout.pos.col = 1, 
                         just = "center"))
pushViewport(viewport(x = 0.36, y = 0.67, width = unit(0.001, "npc")))
grid.draw(regsize_gleg)
pushViewport(viewport(x = 0.6, y = 0.67, width = unit(0.001, "npc")))
grid.draw(veg_gleg)
dev.off()
