...
#pie_min = 0.5 # Minimum diameter of pie. Used in 00_helper_plotting_functions
...
regsize_nudge <- nudge_XY(regsize_df, x = "X", y = "Y", stdvar = "totreg_std2", max_iter = 30, min_shift = 0.4)
names(regsize_nudge)

regsize_nudge <- regsize_nudge %>% mutate(rand_signX = sample(c(-1,1), n(), replace = TRUE),
                                          rand_signY = sample(c(-1,0,1), n(), replace = TRUE),
                                          X_text = ifelse(std_var == 0, X_nudge + 5, 
                                                                        X_nudge + rand_signX*fig_radius),
                                          Y_text = ifelse(std_var > 0 & Y_nudge != Y_orig, 
                                                          Y_nudge + rand_signY*fig_radius, 
                                                          Y_nudge - fig_radius),
                                          fig_area = (fig_radius^2)*pi)
```
...
jpeg("ROVA_new_2n.jpg", height = 8.5, width = 9.9, units = "in", res = 600)
grid.newpage()
pushViewport(map_vp)
...
#upViewport() #goes back to map_vp
pushViewport(map_vp)
